...
from check1 import check_constraint
import math
...
def model(parameters,delta_t = 1):
	...
	T_vec = np.array([Temp(t) for t in np.arange(t0,tf+delta_t,delta_t)])
	...
	while(iteration < num_iter) :
		...
		for t in range(t0,tf,delta_t) :
			...
			T_vec[t] = check_constraint(T_vec[t],y_mat[t,0],DH_vec[iteration,t],M,t,e)
