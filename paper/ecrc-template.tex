...
Optimal control involves the evaluation of time-dependent operating profiles, in terms of the control variable to optimize the process performance. In the crystallization domain Temperature becomes the control variable while the product yield is used to evaluate the performance of the model. The Optimization problem here is solved by using the method of Maximum Principle as discussed extensively by Diwekar\cite{dewikar}. This is prefered over other techniques such as dynamic programing which involves the use of partial differential equations, however maximum principle can be implemented through first order ODEs.   
